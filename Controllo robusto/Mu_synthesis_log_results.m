...
% Nota: ho aggiornato il titolo della figura per riflettere i controllori K_rob e K_red al posto di LQG
figure('Name', 'Confronto Passivo vs K_rob vs K_red', 'Position', [100, 100, 1200, 800]);
...
legend('Passivo', 'K_{rob}', 'K_{red}', 'Location', 'best');
...
legend('u_1 (K_{rob})', 'u_1 (K_{red})', 'u_2 (K_{rob})', 'u_2 (K_{red})', 'Location', 'best');
...
% COMFORT
fprintf('--- COMFORT (Accelerazione zs_ddot) ---\n');
...
% ROAD HOLDING
fprintf('--- ROAD HOLDING (Deformazione delta_t) ---\n');
...
% CORSA SOSPENSIONE
fprintf('--- CORSA SOSPENSIONE (delta_s) ---\n');
fprintf('  PASSIVO     -> Varianza: %.6f | RMS: %.5f m | Max: %+.5f | Err. Finale: %+.5f m\n', var(ds_pass), rms(ds_pass), max(ds_pass), ds_pass(end));
fprintf('  K_rob       -> Varianza: %.6f | RMS: %.5f m | Max: %+.5f | Err. Finale: %+.5f m\n', var(ds_krob), rms(ds_krob), max(ds_krob), ds_krob(end));
fprintf('  K_red       -> Varianza: %.6f | RMS: %.5f m | Max: %+.5f | Err. Finale: %+.5f m\n\n', var(ds_kred), rms(ds_kred), max(ds_kred), ds_kred(end));

% SFORZO DI CONTROLLO
fprintf('--- SFORZO DI CONTROLLO (Attuatori u1, u2)---\n');
fprintf('  K_rob   -> u1 Varianza: %8.1f | u1 RMS: %6.1f N | u1 Max: %+.1f | u1 Min: %+.1f\n', var(u1_krob), rms(u1_krob), max(u1_krob), min(u1_krob));
fprintf('          -> u2 Varianza: %8.1f | u2 RMS: %6.1f N | u2 Max: %+.1f | u2 Min: %+.1f\n', var(u2_krob), rms(u2_krob), max(u2_krob), min(u2_krob));
fprintf('  K_red   -> u1 Varianza: %8.1f | u1 RMS: %6.1f N | u1 Max: %+.1f | u1 Min: %+.1f\n', var(u1_kred), rms(u1_kred), max(u1_kred), min(u1_kred));
fprintf('          -> u2 Varianza: %8.1f | u2 RMS: %6.1f N | u2 Max: %+.1f | u2 Min: %+.1f\n\n', var(u2_kred), rms(u2_kred), max(u2_kred), min(u2_kred));