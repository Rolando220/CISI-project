...
% Metti qui il controllore che vuoi testare (-K_mixsyn, -K_hinf, -K_rob, -K_red)
K_test = -K_red; 

% Adattamento dinamico al numero di sensori (3 o 4)
if size(K_test, 2) == 4
    K_test.u = {'zs_ddot', 'delta_s', 'delta_t', 'zu_ddot'}; 
else
    K_test.u = {'zs_ddot', 'delta_s', 'zu_ddot'}; 
end
K_test.y = {'u1_cmd', 'u2_cmd'};

% 2. Chiusura dell'anello incerto 
% Estraiamo solo le uscite di interesse (zs_ddot e delta_t)
CL_unc = connect(P_esteso, K_test, {'w_in'}, {'zs_ddot', 'delta_t'});

% 3. Campionamento del set di incertezza (15 impianti)
num_campioni = 15;
rng('default'); % Fissiamo il seed per la generazione dei 15 impianti
...
% Rimosso il fattore /dt che amplificava il segnale di 10 volte
rng(42); % Fissiamo il seed del rumore per avere la stessa "strada" ad ogni test
w_in_noise = sqrt(varianza_strada) * randn(length(t_sim), 1);

% 5. Simulazione e Plot 
figure('Name', 'Robustezza Linearizzata (Strada 4)', 'Position', [100, 100, 900, 600], 'Color', 'w');

% Finestra di zoom per vedere i dettagli dei 15 impianti (1 secondo)
zoom_window = [2.0, 3.0];
...
title('Robustezza Comfort ($\ddot{z}_s$) - Scenario 4: Sterrato (Zoom 1s)', 'Interpreter', 'latex', 'FontSize', 12);
...
for i = 1:num_campioni
    % delta_t è la SECONDA uscita estratta dal connect
    [y_dt, ~] = lsim(CL_campioni(2,1,i), w_in_noise, t_sim);
    if i == 1
...
[y_dt_nom, ~] = lsim(CL_nominale(2,1), w_in_noise, t_sim);
p_nom2 = plot(t_sim, y_dt_nom, 'r--', 'LineWidth', 2, 'DisplayName', 'Impianto Nominale');

title('Robustezza Tenuta di Strada ($\delta_t$) - Scenario 4: Sterrato (Zoom 1s)', 'Interpreter', 'latex', 'FontSize', 12);
...
% =========================================================
% Calcolo Analitico Varianza, RMS e Picchi 
% =========================================================
var_acc_campioni     = zeros(1, num_campioni);
rms_acc_campioni     = zeros(1, num_campioni);
var_dt_campioni      = zeros(1, num_campioni);
...
for i = 1:num_campioni
    % Simulazione Comfort
    [y_acc, ~] = lsim(CL_campioni(1,1,i), w_in_noise, t_sim);
    var_acc_campioni(i) = var(y_acc);
    rms_acc_campioni(i) = rms(y_acc);
    
    % Simulazione Tenuta di Strada
    [y_dt, ~] = lsim(CL_campioni(2,1,i), w_in_noise, t_sim);
    var_dt_campioni(i)  = var(y_dt);
    rms_dt_campioni(i)  = rms(y_dt);
    estensione_max_dt(i)   = max(y_dt); 
    compressione_max_dt(i) = min(y_dt); 
end

fprintf('\n--- RISULTATI MONTE CARLO SUI 15 IMPIANTI INCERTI ---\n');
fprintf('COMFORT:\n');
fprintf('  Varianza Accelerazione  (Min / Max): %.4f / %.4f (m/s^2)^2\n', min(var_acc_campioni), max(var_acc_campioni));
fprintf('  RMS Accelerazione       (Min / Max): %.4f / %.4f m/s^2\n', min(rms_acc_campioni), max(rms_acc_campioni));
fprintf('TENUTA DI STRADA:\n');
fprintf('  Varianza Deformazione   (Min / Max): %.6f / %.6f m^2\n', min(var_dt_campioni), max(var_dt_campioni));
